...
// https://davidson.weizmann.ac.il/online/askexpert/%D7%90%D7%99%D7%9A-%D7%9E%D7%98%D7%99%D7%A1%D7%99%D7%9D-%D7%97%D7%9C%D7%9C%D7%99%D7%AA-%D7%9C%D7%99%D7%A8%D7%97
	...
	public static void main(String[] args) {
		double p = 0.00045, i = 0.0000004, d = 0.00000155;
		for (int j = 0; j < 1; j++) {
//			p *= 2;
//			i *= 2;
//			d *= 2;
			PID pid = new PID(p,i,d);
//			PID pid = new PID(0.00045, 0.0000004, 0.00000155);


//			LunarLanderPIDController pid = new LunarLanderPIDController(p, i, d);
			...
			double alt = 13748; // height, 2:25:40 (as in the simulation) // https://www.youtube.com/watch?v=JJ0VfRL9AMs
			double time = 0; // time from started
			double dt = 1; // sec
			double acc = 0; // Acceleration rate (m/s^2)
			double fuel = 121; //
			double weight = WEIGHT_EMP + fuel;
			System.out.println("time, vs, hs, dist, alt, ang,weight,acc");
			double NN = 0.7; // rate[0,1] brakes power
...
//					double output = pid.computeControl(0, ang, dt);
//					System.out.println("pid = " + output);
					double output = pid.compute(alt,dt);
					NN = 1 - output;
					if (hs < 2) {
						hs = 0;
					}
